...
- Determining whether or not a graph has a Hamilton circuit is a question that can be answered with mathematical propositions, such as [[Dirac's Theorem]] and [[Ore's Theorem]]. These give conditions which, when met, guarantee the existence of a Hamilton circuit. But they do not 
- The problem of finding a Hamilton circuit in a graph, on the other hand, is [NP-complete](https://en.wikipedia.org/wiki/NP-completeness). In layperson's terms this means that while it is computationally easy to *verify* whether a given cycle is Hamiltonian, there is no known way to *find* such a cycle quickly. (Contrast with the similar problem for [[Eulerian trail and circuit|Eulerian circuits]], where there is a simple theorem that will detect the existence of such circuits.)
