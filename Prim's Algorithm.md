...
- Look at all edges that come out from the tree with one vertex in the tree and the other not in the tree: $\{0,8\},\{3,2\}$ and $\{1,7\}$. The one with minimum weight is $\{0,8\}$ so add it to the tree: $\{\{0,1\}, \{0,3\}, \{3,4\}, \{0,8\}\}$. 
- Look at all edges that come out from the tree with one vertex in the tree and the other not in the tree: $\{0,8\},\{1,7\}, \{3,2\}$, and $\{4,8\}$. There's a tie for lowest weight between $\{0,8\}$ and $\{1,7\}$ so choose one. Let's say 
